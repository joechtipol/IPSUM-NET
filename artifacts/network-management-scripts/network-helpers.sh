...
instantiateChaincode () {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
    peer chaincode instantiate -o orderer.ipsum.io:7050 -C $CHANNEL_NAME -n ipsumcc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":[]}' -P "OR  ('auditorMSP.peer','univ1MSP.peer','univ2MSP.peer')" >&log.txt
    res=$?
                set +x
  else
                set -x
    peer chaincode instantiate -o orderer.ipsum.io:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ipsumcc -l ${LANGUAGE} -v 1.0 -c '{"Args":[]}' -P "OR ('auditorMSP.peer','univ1MSP.peer','univ2MSP.peer')" >&log.txt
    res=$?
                set +x
  fi
  cat log.txt
...
upgradeChaincode () {
    ...
    set -x
    peer chaincode upgrade -o orderer.ipsum.io:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ipsumcc -v 2.0 -c '{"Args":[]}' -P "OR ('auditorMSP.peer','univ1MSP.peer','univ2MSP.peer')"
    res=$?
  set +x
    cat log.txt
